...
function extractDomainsFromCommand(command: string): string[] {
  ...
  while ((match = urlRegex.exec(command)) !== null) {
    try {
      const host = new URL(match[0]).hostname.replace(/^\[(.*)\]$/, "$1");
      if (host) domains.add(host);
    } catch {
      // Ignore malformed URLs in shell text.
    }
  }
  return [...domains];
}

function cidrMatchesIp(ip: string, cidr: string): boolean {
  const [network, prefixRaw] = cidr.split("/");
  const prefix = Number(prefixRaw);
  const family = isIP(network);
  if (!family || !Number.isInteger(prefix)) return false;
  if (isIP(ip) !== family) return false;
...
function domainMatchesPattern(domain: string, pattern: string): boolean {
  const normalizedDomain = domain.replace(/^\[(.*)\]$/, "$1").toLowerCase();
  const normalizedPattern = pattern.toLowerCase();

  if (normalizedPattern === "*") return true;
  if (normalizedPattern.includes("/")) return cidrMatchesIp(normalizedDomain, normalizedPattern);
  if (isIP(normalizedPattern)) return normalizedDomain === normalizedPattern;
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (isSshModeActive()) {
      await disableSandbox(ctx);
      ctx.ui.notify("Sandbox disabled because --ssh is active", "info");
      return;
    }
  ...
  pi.registerCommand("sandbox-enable", {
    ...
    handler: async (_args, ctx) => {
      ...
      if (isSshModeActive()) {
        ctx.ui.notify("Sandbox cannot be enabled while --ssh is active", "warning");
        return;
      }
